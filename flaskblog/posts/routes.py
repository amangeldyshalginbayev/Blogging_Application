    ...
    if form.validate_on_submit():
        if form.image.data:
            ...
            post = Post(title=form.title.data, content=form.content.data,
                        author=current_user, image_file=post_image)
        else:
            post = Post(title=form.title.data, content=form.content.data,
                        author=current_user)
        db.session.add(post)
...
def post(post_id):
    ...
    comments_of_post = Comment.query.filter_by(post_id=post.id). \
        order_by(Comment.date_commented.desc()).all()

    return render_template("posts/post.html", title=post.title, post=post,
                           form=form, comments=comments_of_post)
