...
def test_create(client, authentication, app_with_test_data_in_db):
    with app_with_test_data_in_db.app_context():
        ...
        client.post("/post/new", data=dict(title="New Post Title", content="New Post Content", follow_redirects=True))
...
def test_update(app_with_test_data_in_db, authentication, client):
    with app_with_test_data_in_db.app_context():
        ...
        client.post("/post/1/update", data=dict(title="New Updated Title", content="New Updated Content", follow_redirects=True))
        index_page = client.get("/")
...
def test_delete(app_with_test_data_in_db, authentication, client):
    with app_with_test_data_in_db.app_context():
        authentication.login()
        client.post("/post/1/delete", data=dict(title="New Updated Title", content="New Updated Content", follow_redirects=True))
        index_page = client.get("/")
        assert not b"test_title1" in index_page.data
        assert not b"test_content1" in index_page.data

def test_user_can_not_update_other_users_post(app_with_test_data_in_db, authentication, client):
    with app_with_test_data_in_db.app_context():
        authentication.login()
        client.get("/post/2/update").status_code == 403

def test_user_can_not_delete_other_users_post(app_with_test_data_in_db, authentication, client):
    with app_with_test_data_in_db.app_context():
        authentication.login()
